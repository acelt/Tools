...
# Heres a funciton to pull out and tidy up benchmarks to benchmarki tool format
tidy_bm <- function(data,
                    indicators = c("bareGround" = "BareSoilCover",
                                   "soilStability" = "SoilStability_All",
                                   "littercover" = ""),
                    inclusive = TRUE){
  # data = the dataframe you are tidying, must have a cols of indicators
  # Indicators is a named vector in teh format of indicator column name = terrdat indicator column name, the defaults are those used in EDIT ref sheets
  # Inclusive specifys if the benchmarks in the input data include or exclude the upper and lower bounds (e.g., < or <=)
  
  # is it possible to create benchmarks for functional groups? might be tricky with current objective formula?
}
...
grab_referencesheets <- function(mlra, out_path = wd()){
  ...
  # might be good to add a link to this reference sheet to
  
  # Then write it to a flat csv file
  write.csv(final_data, paste0(out_path, mlra, "_final_data.csv"))
  
  # Also reorganise the table to match benchmark tool and pull out benchmarks
