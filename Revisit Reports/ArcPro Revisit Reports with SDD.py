...
# output folder as a tool input here
...
prt("Got list of PrimaryKeys - " + str(len(terrapkList)) + " plots from TerraDat and " + str(len(lmfpkList)) + " LMF plots")

# Would be good to get plot service for the relevant state in statelist above
state_plot_service = arcpy.GetParameterAsText(2) # could also do this as a drop down for states but many of them will change each year
...
if len(stateList)>1 & "CO" in statelist:
...
if statelist == "CO":
    ...
    SDD = "\\blm.doi.net\dfs\loc\EGIS\ProjectsNational\AIM\AIMDataTools\SDE\AIMDev.sde"
...
terraSDE = SDD + "\\" + terraFC # switching out for AIMdev to get designpointkeys
...
if statelist == "CO":
    ...
    designPointsFC = SDD + "\\" + designPoints
else:
...
for pk in terrapkList:
    ...
    if "CO" in stateList :
        ...
        designPoints_df_pk = designPoints_df.loc[designPoints_df.PrimaryKey == pk, ['PlotID', 'DesignName', 'DesignPointKey']]

    # I dont think we need the following if just using a layer file
    # Grab the plotid from the plots service instead of SDD in case of name changes
    gis = GIS('pro')
    item = gis.content.get(state_plot_service)
    flayer = item.layers[0]
    plot_service_df = pd.DataFrame.spatial.from_layer(flayer)
    plot_service_df_pk = plot_service_df.loc[plot_service_df.V1PrimaryKey == pk, ['PlotID', 'V1PrimaryKey']]

    try:
        plotid = plot_service_df_pk['PlotID'].values[0]
    except:
        plotid = pk

    print("Building Revisit Report for " + pk + " also known as "+ plotid)
    ...
    arcpy.AddJoin_management("terraLayer", "PrimaryKey", "designLayer", "PrimaryKey")
    ...
    fields = [terraFC + '.PrimaryKey', terraFC + '.PlotID', terraFC + '.Latitude_NAD83', terraFC + '.Longitude_NAD83', terraFC + ".State", plotsFC + ".Elevation", plotsFC + ".Slope",
            plotsFC + ".Aspect", plotsFC + ".LandscapeType", plotsFC + ".LandscapeTypeSecondary", plotsFC + ".EcolSite", plotsFC + ".Soil", plotsFC + ".Directions", designPoints + ".V1PrimaryKey", designPoints + ".PlotID"]
    ...
    with arcpy.da.SearchCursor("terraLayer", fields, whereClause) as cursor:
        for row in cursor:
            ...
            # Start the PDF
            pdfOut = canvas.Canvas(outputFolder + "\\" + "Revisit Report for " + plotid + ".pdf")
            ...
            lineOffset = 760
            pdfOut.drawString(200,792,"Revisit report for PlotID " + row[1])
            ...
            pdfOut.drawString(20,lineOffset-240,"Directions: ")
            # have to worry about long strings here
            lineOffset = lineOffset-260
    ...
    if len(designPoints_df_pk)>0 and designPoints_df_pk['V1PrimaryKey'].values[0] != 'NA':
        pdfOut.drawString(20, lineOffset, "Visit Number: " + str(designPoints_df_pk.Visit.values[0]))
        pdfOut.drawString(20, lineOffset-20, "Date Visited: "+ str(designPoints_df_pk.DateVisited.values[0]))
        pdfOut.drawString(20, lineOffset-40, "Visit 1 Primary Key: " + str(designPoints_df_pk.V1PrimaryKey.values[0]))
        pdfOut.drawString(20, lineOffset-60, "Sample Design Comments: " + str(designPoints_df_pk.Comments.values[0]))

        designPoints_df_v1 = designPoints_df.loc[designPoints_df.V1PrimaryKey == designPoints_df_pk.V1PrimaryKey.values[0],['PrimaryKey', 'DateVisited', 'Visit']]
